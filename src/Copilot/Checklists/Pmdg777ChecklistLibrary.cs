    ...
    public static IReadOnlyList<ChecklistDefinition> GateToGate { get; } =
        new[]
        {
            Checklist("power-up-initial-setup", "777 Power Up & Preliminary Preflight Verification",
                ...
                Item("Secondary external power", "ON", state => state.Pmdg777SecondaryExternalPowerOn),
            ...
            Checklist("approach-landing", "777 Approach & Landing Verification",
                Item("FMC landing data", "SET", state => state.Pmdg777FmcLandingFlaps is 20 or 25 or 30 && state.Pmdg777FmcLandingVref > 0),
                Item("Landing gear", "DOWN", state => state.Pmdg777GearLeverDown),
                Item("Landing flaps", "SET", state => state.Pmdg777LandingFlapsSet),
                Item("Speedbrake", "ARMED", state => state.Pmdg777SpeedbrakeArmed),
                Item("Autobrake", "2", state => state.Pmdg777AutobrakeSelector == 4),
                Item("Landing lights", "ON", state => state.Pmdg777LandingLightsOn)),
            Checklist("after-landing-taxi", "777 After Landing Verification",
                Item("Aircraft", "ON GROUND", state => state.OnGround),
                Item("Exterior lights", "TAXI", state => state.Pmdg777AfterLandingLightsSet),
                Item("Flaps / speedbrake", "UP / DOWN", state => state.Pmdg777FlapsUp && state.Pmdg777SpeedbrakeDown),
                Item("Transponder", "STBY", state => state.Pmdg777TransponderStandby),
                Item("APU", "POWER / AIR", state => state.Pmdg777ApuGeneratorPowerEstablished && state.Pmdg777ApuBleedAirAvailable)),
            Checklist("parking-shutdown", "777 Shutdown & Secure Verification",
                ...
                Item("Parking brake", "SET", state => state.ParkingBrakeSet),
                Item("Electrical power", "ESTABLISHED", state => state.Pmdg777ApuGeneratorPowerEstablished || state.Pmdg777ExternalPowerOn),
                Item("Engines", "OFF", state => state.EnginesOff),
                Item("Beacon", "OFF", state => !state.Pmdg777BeaconOn),
                Item("Fuel pumps", "OFF", state => state.Pmdg777FuelPumpsOff),
                Item("Hydraulic pumps", "OFF", state => state.Pmdg777HydraulicsShutdown))
        };
